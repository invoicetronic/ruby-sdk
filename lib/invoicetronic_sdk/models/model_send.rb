  ...
  class ModelSend < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Invoicetronic_Sdk::ModelSend` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Invoicetronic_Sdk::ModelSend`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'latest_state')
        self.latest_state = attributes[:'latest_state']
      end
    ...
    def valid?
      ...
      latest_state_validator = EnumAttributeValidator.new('String', ["Inviato", "Consegnato", "NonConsegnato", "Scartato", "AccettatoDalDestinatario", "RifiutatoDalDestinatario", "ImpossibilitaDiRecapito", "DecorrenzaTermini", "AttestazioneTrasmissioneFattura"])
      return false unless latest_state_validator.valid?(@latest_state)
      true
    end
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] latest_state Object to be assigned
    def latest_state=(latest_state)
      ...
      unless validator.valid?(latest_state)
        fail ArgumentError, "invalid value for \"latest_state\", must be one of #{validator.allowable_values}."
      end
      @latest_state = latest_state
    end
    ...
    def hash
      [id, created, version, user_id, company_id, committente, prestatore, identifier, file_name, format, payload, last_update, date_sent, documents, encoding, nome_committente, meta_data, latest_state, company].hash
    end
